...
const Button = ({title, type, textColor, leftIcon, isSubmitting, handleClick, bgColor, rightIcon}: ButtonProps) => {
  return (
    <button type={type || "button"} disabled={isSubmitting} className={`flexCenter gap-3 py-3 px-4 ${textColor || 'text-white'} ${isSubmitting ? 'bg-black/50' : bgColor || 'bg-purple-500'} rounded-xl text-sm max-md:w-full font-medium`}>
